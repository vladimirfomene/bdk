    ...
    A: for<'b> From<AnchorParts<'b>>,
{
        ...
        A: for<'b> From<AnchorParts<'b>>,
    {
        ...
        let txs = block.txdata.iter().enumerate().map(|(tx_pos, tx)| {
            let anchor_parts = (&block, block_id, tx_pos);
            (tx, core::iter::once(A::from(anchor_parts)), None)
        });
...
/// Positional index of a transaction in a block.
pub type TxPosition = usize;

/// Helpful data for constructing an [`Anchor`] from a block.
///
/// Methods [`apply_block_relevant`] and [`apply_block`] of [`IndexedTxGraph`] can only be called if
/// it's anchor implementation (`A`) also satisfies [`From<AnchorParts>`].
///
/// [`apply_block_relevant`]: IndexedTxGraph::apply_block_relevant
/// [`apply_block`]: IndexedTxGraph::apply_block
pub type AnchorParts<'b> = (&'b bitcoin::Block, BlockId, TxPosition);
